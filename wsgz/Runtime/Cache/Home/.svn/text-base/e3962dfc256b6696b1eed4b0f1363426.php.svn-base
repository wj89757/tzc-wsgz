        ...
        <div class="main">
          <div class="main-h" >
            <ul class="breadcrumb">
              ...
              <li class="active">政策文件上传</li>
            </ul>
            <h3 class="side-lined" style="color:#000"><?php echo ($username); ?>欢迎你！</h3>
            <form class="frm-type">
              <a href=<?php echo U('Personal/documentadd');?>>
                <input type="button" class="btn btn-default" value="+添加政策文件"></a>
              <ul class="list-group">
                <?php if(is_array($list)): foreach($list as $key=>$obj): ?><li class="list-group-item">
                    <a href=<?php echo U('Personal/documentres',array('id'=> $obj['documents_id']));?> style="color:#663333">
                              <?php echo ($obj["documents_time"]); ?>【<?php echo ($obj["documents_title"]); ?>】
                      <a  onClick="return confirm('确认要删除该条政策文件？')" href=<?php echo U('Personal/documentdel',array('id'=> $obj['documents_id']));?> title="删除政策文件" data-toggle="tooltip" >
                        <span class="glyphicon glyphicon-trash" style=" float:right;"></span>
                      </a>
                      <a href=<?php echo U('Personal/documentres',array('id'=> $obj['documents_id']));?> title="修改政策文件">
